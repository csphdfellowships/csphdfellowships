...
//     }
    ...
    for (i = 0; i < temp_data.length; i++) {
        ...
        $("#ranking").find("tbody").append(
            ...
            .append($("<td>").append("&nbsp;").text(new Date(get_deadline(temp_data[i])).toDateString().split(' ')[1] + " " + new Date(get_deadline(temp_data[i])).toDateString().split(' ')[2] + " " + new Date(get_deadline(temp_data[i])).toDateString().split(' ')[3]).attr("align", "right"))
            ...
            .append($("<td>").text(get_fellowship_type(temp_data[i]).toLocaleString("en-US")).attr("align", "right"))
            .append($("<td>").text(get_citizenship(temp_data[i]).toLocaleString("en-US")).attr("align", "right"))
            )
